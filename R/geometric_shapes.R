...
create_geometric_shapes <- function(font = "square") {
	...
	# 25a0 black square
	d_25a0 <- d_rect(x = xc, y = yc, w = cw, h = cw)
	write_svg(d_25a0, "25a0")
	# 2b1b black large square
	d_2b1b <- d_rect(x = xc, y = yc, w = w, h = w)
	write_svg(d_2b1b, "2b1b")

	d_25fc <- d_rect(x = xc, y = yc, w = cdw, h = cdw)
	d_25fb <- d_rect(x = xc, y = yc, w = cdw - 2 * ow, h = cdw - 2 * ow)
	...
	# 25a1 white square
	d_25a1 <- d_rect(x = xc, y = yc, w = cw - 2 * ow, h = cw - 2 * ow)
	...
	# 25c6 black diamond
	d_25c6 <- d_diamond(xc, yc, cw, cw)
	write_svg(d_25c6, "25c6")
	# 25c7 white diamond
	d_25c7 <- d_diamond(xc, yc, cw, cw, offset = -ow)
	write_svg(d_25c6 + d_25c7, "25c7")

	# 2b25 black medium diamond
	d_2b25 <- d_diamond(xc, yc, w = cdw, h = cdw)
	write_svg(d_2b25, "2b25")
	# 2b26 black medium diamond
	d_2b26 <- d_diamond(xc, yc, w = cdw, h = cdw, offset = -ow)
	write_svg(d_2b25 + d_2b26, "2b26")

	d_25cf <- d_circle(xc, yc, 0.5 * cw)
	d_2b24 <- d_circle(xc, yc, 0.5 * w)

	r_inner <- 0.5 * cw - ow
	d_25cb <- d_circle(xc, yc, r_inner)
	d_25ef <- d_circle(xc, yc, 0.5 * w - ow)

	# 25cb white circle
	write_svg(d_25cf + d_25cb, "25cb")
	# 25ef large circle
	write_svg(d_2b24 + d_25ef, "25ef")

	# 25cf black circle
	write_svg(d_25cf, "25cf")
	# 2b24 black large circle
	write_svg(d_2b24, "2b24")
	...
	l <- list(
		x = c(xc - 0.5 * cw, xc + 0.5 * cw, xc),
		y = c(yc + 0.5 * ch, yc + 0.5 * ch, yc - 0.5 * ch)
	)
	d_25bc <- d_polygon(l)
	write_svg(d_25bc, "25bc")
	# 25bd white down-pointing triangle
	d_25bd <- d_polygon(l, offset = -ow)
	write_svg(d_25bc + d_25bd, "25bd")

	# 25e2 black lower right triangle
	l <- list(x = hg + c(0, cw, cw), y = c(yc - 0.5 * cw, yc + 0.5 * cw, yc - 0.5 * cw))
	d_25e2 <- d_polygon(l)
	...
	l <- list(x = hg + c(0, 0, cw), y = c(yc - 0.5 * cw, yc + 0.5 * cw, yc - 0.5 * cw))
	d_25e3 <- d_polygon(l)
	...
	write_svg(c(white_square, d_rect(hg + 0.25 * cw, yc, 0.5 * cw, cw)), "25e7")
	# 25e8 square with right half black
	write_svg(c(white_square, d_rect(hg + 0.75 * cw, yc, 0.5 * cw, cw)), "25e8")
	...
	as_hex(c(
		as.hexmode(c("25a0", "25a1", "25a3", "25c6", "25c7", "25cb")),
		as.hexmode("25cf"):as.hexmode("25d3"),
		as.hexmode("25ef"),
		as.hexmode("25fb"):as.hexmode("25fc"),
		as.hexmode("25bc"):as.hexmode("25bd"),
